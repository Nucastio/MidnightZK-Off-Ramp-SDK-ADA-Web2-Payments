...
async function main(): Promise<void> {
  ...
  // Periodic sync-progress logger: subscribes to the wallet state stream and
  // prints applied/relevant/highest indices for shielded/unshielded/dust every
  // ~5s. Returns an unsubscribe fn.
  function logProgress(label: string, w: { state(): rx.Observable<unknown> }) {
    let last = 0;
    const sub = (w.state() as rx.Observable<{ shielded: { progress: unknown }; unshielded: { progress: unknown }; dust: { progress: unknown }; isSynced: boolean }>).subscribe((s) => {
      const now = Date.now();
      if (now - last < 5000 && !s.isSynced) return;
      last = now;
      const fmt = (p: unknown) => {
        const x = p as { appliedIndex: bigint; highestRelevantIndex: bigint; highestIndex: bigint; isConnected: boolean };
        return `applied=${x.appliedIndex} rel=${x.highestRelevantIndex} highest=${x.highestIndex} conn=${x.isConnected}`;
      };
      console.log(`[sync ${label}] synced=${s.isSynced} | shielded[${fmt(s.shielded.progress)}] | unshielded[${fmt(s.unshielded.progress)}] | dust[${fmt(s.dust.progress)}]`);
    });
  ...
  const stopSenderLog = logProgress("genesis", sender.wallet);
  ...
  const stopReceiverLog = logProgress("receiver", receiver.wallet);
